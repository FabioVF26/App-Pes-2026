...
fun NuovoSopralluogoHaccpScreen(
    ...
    LazyColumn(
        ...
        item {
            Text("Nuovo sopralluogo HACCP", style = MaterialTheme.typography.headlineSmall, color = BrandNavy)
            Text(
        ...
        if (clienteSelezionato != null && !mostraRicerca) {
            ...
            if (ricerca.trim().length >= 2) {
                if (risultati.isEmpty()) {
                    item { Text("Nessun cliente HACCP trovato.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                } else {
        ...
        if (clienteId > 0) {
            ...
            item {
                Button(
                    modifier = Modifier.fillMaxWidth(),
                    enabled = sedeId > 0,
                    onClick = { viewModel.creaSopralluogoHaccp(clienteId, sedeId, note, onCreated) }
                ) {
